...
def fine_tune(train_df, valid_df, checkpoints_path, id2label, label2id, model):
    ...
    class CNNModel(AutoModelForSequenceClassification):
        def __init__(self, config):
            ...
            self.classifier = nn.Linear(128 * 2, len(label2id))
    ...
    class CRFModel(CNNModel):
        def __init__(self, config):
            super().__init__(config)
            self.crf = CRF(num_tags=len(label2id))

        def forward(self, input_ids, attention_mask=None):
            logits = super().forward(input_ids, attention_mask=attention_mask)[0]
            loss = self.crf(logits, attention_mask)
            return {"loss": loss}


    model = CNNModel.from_pretrained(model)
